...
export default class Hashtags extends Component {
  ...
  getPosts(){
    ...
    fetch(global.baseurl+'get_hash_posts.php?limit='+this.state.count+'&hash='+encodeURI(this.state.pet),{
     method:"GET",
    })
    .then((response) => response.json())
    .then((responseJson) => {
  ...
  componentDidMount = () => {
    ...
    fetch(global.baseurl+'getpopulathashtags.php',{
     method:"GET",
    })
    .then((response) => response.json())
    .then((responseJson) => {
      ...
      <View style={styles.container}>
       ...
       <Modal
          ...
          <View style={{marginTop: 22, flex: 1,position: 'absolute',width:'100%',left: 0,right: 0,bottom: 0,}}>
           ...
           <ScrollView horizontal={true} showsHorizontalScrollIndicator={false} style={{margin:0}} >
            {this.state.tags.map((item,i)=> {
              return(
                <TouchableOpacity onPress={()=>this.sendhash(item)}>
                  <Text style={this.state.pet == item ? styles.contents : styles.content}>#{item}</Text>
                </TouchableOpacity>
                )} ) 
              }
           </ScrollView>    
